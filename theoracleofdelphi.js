...
define([
    ...
    "ebg/core/gamegui",
    "ebg/counter",
    g_gamethemeurl + "modules/js/HexGrid.js?v480",
    g_gamethemeurl + "modules/js/Components.js?v480",
    g_gamethemeurl + "modules/js/ClusterDefinitions.js?v480",
    g_gamethemeurl + "modules/js/BoardBuilder.js?v480",
    g_gamethemeurl + "modules/js/BoardRenderer.js?v480",
    g_gamethemeurl + "modules/js/LogGlyphs.js?v480",
    g_gamethemeurl + "modules/js/LogTokens.js?v480",
    g_gamethemeurl + "modules/js/DeliveryRelations.js?v480",
    g_gamethemeurl + "modules/js/ZeusTaskTargets.js?v480",
    g_gamethemeurl + "modules/js/ShrineTaskTargets.js?v480",
    g_gamethemeurl + "modules/BX/js/DragScroller.js?v480",
],
    ...
    '</div>';
        ...
        */
        ...
        setZoomBalance: function(pct, opts) {
            ...
            // are bound to the region and the document, so they would otherwise
            // still drive a control the player cannot see.
        ...
        _updateGameScale: function() {
            ...
            // multiplier to compose with here: stacked, the zoom is hidden and
            // held at neutral, so this is the auto-fit on its own.
         ...
         *
         * Stacked, the two regions sit in separate rows and do not compete for
         * width, so there is nothing for a balance between them to trade. The
         * control is hidden there, and any zoom already applied is returned to
         * neutral rather than left stuck on with no visible way to undo it.
         *
         * The reset is persisted so storage always matches what is on screen and
        ...
        setupZoomControls: function() {
            ...
            // Reachable from _syncZoomAvailability, which has to close the panel
            // when the layout stops offering the zoom at all.
            this._setZoomPanelOpen = setOpen;
        ...
        onHexClick: function(q, r, type, color) {
            ...
            // Click-to-build and click-to-explore via the island tile. The
            // gold overlay is a 54px circle on a 60x69 hex, so a click on the
            ...
            // Explore goes through the shared handler rather than dispatching
            // directly, so an island that can still be looked at raises the
            // explore-vs-peek confirm. That check is why explorable hexes are
        ...
        _openAutoDefeatConfirm: function(monsters) {
            ...
            monsters.forEach(function(m) {
                var typeCap = m.type
                    ...
                    : _('Monster');
                // Matches the everyday End Turn button (PlayerActions, actions
                // still left), per G's call. Explicitly NOT red: red is this
                // file's dismiss colour (_addCancelButton), so red here
                // rendered the two committing buttons identically to the
                // Cancel beside them — in the one dialog that exists because
                // a player already clicked the wrong monster on a shared hex.
                // The per-monster portrait icon below is what tells them
                // apart; button colour deliberately stays out of it, since in
                // the normal Fight flow a monster's colour IS the die colour
                // required and Ares spends no die at all.
        ...
        onUpdateActionButtons: function( stateName, args )
        {
            ...
            // Same lifecycle for the island-tile click routes onHexClick reads:
                ...
                switch( stateName )
                {
                    ...
                    case 'SelectAction':
                        ...
                        if (args && args.explorableIslands && args.explorableIslands.length > 0) {
                            ...
                            // island hasn't been peeked. onHexClick reads the
                            // same map, so a click anywhere on the island tile
                            // takes the identical route.
                            this._highlightValidHexes(
                        ...
                        if (args && args.buildableShrines && args.buildableShrines.length > 0) {
                            // Whole-island click target: onHexClick dispatches
                            // actBuildShrine from this map when the click lands
                            // on the shrine tile beside the gold disc.
                            this._buildableShrineHexKeys = new Set(
                                args.buildableShrines.map(
                                    s => parseInt(s.hex_q) + ',' + parseInt(s.hex_r)
                                )
                            );
                            this._highlightValidHexes(
        ...
        getGodAbilityDescription: function(ability) {
            switch (ability) {
                ...
                case 'dice_wild': return _('All dice wild + draw an Oracle card; Oracle Card played this turn is also wild');
        ...
        _highlightValidHexes: function(hexes, className, onClick, tooltip) {
            ...
            hexes.forEach(function(hex) {
                ...
                overlay.addEventListener('click', function(e) {
                    // onHexClick now routes build/explore by hex as well, and
                    // the board container resolves stray clicks by pixel. Let
                    // this one bubble and the same click dispatches twice.
                    if (e && e.stopPropagation) e.stopPropagation();
                    onClick(q, r);
                });
