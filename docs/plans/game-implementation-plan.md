...
- [x] **State machine** — 30 state classes in `modules/php/States/`, auto-discovered by BGA framework (no `states.inc.php`). Happy-path turn loop: RoundStart→PlayerTurnStart→CheckInjuries→PlayerActions→ConsultOracle→NextPlayer→loop. Round 1 for the `fewer_tasks` ship-tile holder detours through `DiscardZeusTile` before `PlayerTurnStart`. [L]
...
- [x] Titan attack at round end (roll titan die, compare shields) [M]
  ...
  - **Titan holder**: stays with the last player for the entire game per Oracle of Delphi rules. `titan_holder_id` is set once at `setupNewGame` and never rotates; no pass notification.
  - Notifications split public/private: `titanRoll` public (value), `titanInjuryPrivate` (card_id + color to affected player only), `titanInjury` public (player + count + colors, no card_ids), `titanNoInjury` public (shield held), `injuryDeckReshuffled` public.
...
- [x] Ship tile abilities (8 types — see Section 5.3) [XL]
  - `shield_start`, `favor_plus_1`, `god_track_high`, `starting_equipment`, `range_plus_2` wired up in setup/movement.
  - `fewer_tasks` (tile 6, 11 tasks to win): new `DiscardZeusTile` active-player state (id 5) that runs once between setup and round 1 when a player is dealt this tile. The player clicks one of their 12 Zeus tiles to return to the box; the row is `DELETE`d so end-game eligibility, display, and the `EndScore` tie-break aux score all adapt naturally without schema changes. `RoundStart.onEnteringState` detours to `DiscardZeusTile` whenever the holder still has 12 rows.
...
- [ ] Equipment card effects (22 cards) [XL]
  ...
  - Batch 2 — cards 014, 015, 016 (Shallow Runner, Pain Tolerance, Reinforced Hull) landed in commit `<SHA>`.
- [x] Companion abilities (18 cards) [L]
  ...
  - **Creature** (subtype 0, `move_range_plus_3`): when the active die color matches a Creature the player owns, `MoveShip` adds 3 to base range AND lifts the destination-color restriction. `getMovementRange`/`getMaxMovementRange` take a `$dieColor`, `getArgs` computes a `creatureActive` flag that bypasses the hex-color filter, and `actConfirmMove` applies the same bypass on destination validation. A `creatureMoveBonus` notif fires from `MoveShip.onEnteringState` so the log announces the bonus the moment Move Ship opens.
  - **Demigod** (subtype 1, `die_wild_color`): on acquire, draws 1 Oracle Card from the top of the deck. Ongoing: when a **die** (not an oracle card) of the Demigod's color is selected, the player may use it as any color — piggybacked on the existing Recolor Die flow with a new `demigodWild` flag that makes the recolor free. Recolor UI shows a dedicated "Use your ${color} Demigod to treat the die as any color" status title. Apollo supersedes Demigod when both would apply.
  - Shared helper: `Game::playerOwnsCompanion($pid, $color, $typeIdx)` (with `playerOwnsHero` delegating) is the single source of truth for ownership checks across all three abilities.
  - Pre-existing bug fix bundled in: `setupHandCardsFromGamedata` was reading `colors[cardTypeArg]` for companion cards on reload — wrong for companions since `cardTypeArg = color_idx * 3 + type_idx`. Corrected to `colors[floor(cardTypeArg/3)]`.
- [x] End game detection (all tasks + return to Zeus) [M]
  - `MoveShip.isEligibleForZeus()` checks for zero incomplete `zeus_tile` rows. When eligible, pathfinder unlocks the Zeus shallows hex; entry is color-agnostic. Landing on Zeus appends the player to the `zeus_reachers` global and (on the first Zeus landing) sets `winner_player_id` to trigger final-round rotation. The winning player's own turn completes normally via `spendActionSource`; `NextPlayer` detects round completion when turn rotation returns to the first Zeus-reacher and transitions to `PreEndGame`. Additional players may also reach Zeus during the final round — `EndScore` handles the tie-break. `fewer_tasks` (11-tile win) works for free because eligibility reads the row count from the `zeus_tile` table, and the `DiscardZeusTile` state deletes the row at setup.
